  ...
  componentDidMount() {
    this.setState(
      ...
      () => {
        ...
        console.log(newarr);
        const newarr2 = newarr.sort(function (a, b) {
          //평수 작은 순서대로 정렬
          if (a > b) return 1;
          if (a === b) return 0;
          if (a < b) return -1;
          ...
          <div className="meme_1">
            <div className="meme_chart_main">
              <div className="kakao_road_view">
                ...
                <a href={this.state.road_view} target="_blank">
                  주변 카카오 로드뷰 보기
                </a>
              </div>
              <div className="meme_chart_title">
                ...
                <div className="avg_meme_div">
                  <div className="avg_meme_div1">최근 거래 가격 기준</div>
                  <div className="avg_meme_div2">{this.state.avg_meme}</div>
                </div>
  ...
  render() {
    return (
      <div className="pho_main">
        <div className="pho_title">최근 거래된 목록</div>
        <div className="pho_main_title">
          <div className="pho_main_title_row">계약일</div>
          <div className="pho_main_title_row">가격</div>
          <div className="pho_main_title_row">전용평수</div>
          <div className="pho_main_title_row">층</div>
        </div>
        {this.props.make_data2.map((v, i, a) => {
          if (i < this.state.click) {
            return <Meme_chart_row date={v.date} price={v.price} size={v.size} floor={v.floor} />;
          }
        })}
        {this.props.make_data2.length > 5 ? (
          <div className="pho_main_title_row_button">
            <button onClick={this.onclicker}>
              {this.state.name}
              <img src={this.state.img} width="10px" height="10px" />
            </button>
            <div className="gonbak"></div>
          </div>
        ) : (
          <div></div>
        )}
    ...
    return (
      <div>
        <div className="pho_main_row_main">
          <div className="pho_main_row">{this.props.date}</div>
          <div className="pho_main_row">{this.state.price1}</div>
          <div className="pho_main_row">{Math.floor(this.props.size / 3.3058)}평</div>
          <div className="pho_main_row">{this.props.floor}</div>
        </div>
      </div>
    );
  }
